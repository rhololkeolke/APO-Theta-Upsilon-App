			...
			if(result != null)
			{
				try {
					...
					if(requestStatus.compareTo("valid login") == 0)
					{
						...
						if(!Auth.setOtpAndHmac(result.getString("key"), result.getString("iv")))
						{
							Toast msg = Toast.makeText(getApplicationContext(), "Error setting OTP and HMAC", Toast.LENGTH_LONG);
							msg.show();
						}
